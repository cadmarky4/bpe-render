...
if (isset($SEC_check_config)) {
	...
	if ($SEC_check_config == 'ENV_File_Missing') {
		...
		$DB_CONFIG_Default = [
			...
			'DB_Name' => 'railway',
			'DB_User' => 'root',
			'DB_Pass' => 'SPXnYpJhsopTzEsBBwFHuiRQbXerPQtH',
		];
	...
	if (isset($_POST['config_type'])) {
		...
		if ($_POST['config_type'] == 'Config_Test_Database') {
			...
			try {
				...
				$host = $_POST['DB_Host'] ?? (getenv('MYSQL_HOST') ?: 'ballast.proxy.rlwy.net');
				$port = getenv('MYSQL_PORT') ?: '33860';
				$user = $_POST['DB_User'] ?? (getenv('MYSQL_USER') ?: 'root');
				$password = $_POST['DB_Pass'] ?? (getenv('MYSQL_PASSWORD') ?: 'SPXnYpJhsopTzEsBBwFHuiRQbXerPQtH');
				...
				if (!file_exists($DB_Migration_File)) {
					...
					if ($DB_Migration_File_content) {
						...
						try {
							...
							$host = getenv('MYSQL_HOST') ?: 'ballast.proxy.rlwy.net';
							$port = getenv('MYSQL_PORT') ?: '33860';
							$database = getenv('MYSQL_DATABASE') ?: 'railway';
							$user = getenv('MYSQL_USER') ?: 'root';
							$password = getenv('MYSQL_PASSWORD') ?: 'SPXnYpJhsopTzEsBBwFHuiRQbXerPQtH';
		...
		elseif ($_POST['config_type'] == 'APP_Admin_User_Save') 
		{
				...
				$_POST['Admin_Email'] != '') 
			{
				...
				$database = getenv('MYSQL_DATABASE') ?: 'railway';
				$user = getenv('MYSQL_USER') ?: 'root';
				$password_db = getenv('MYSQL_PASSWORD') ?: 'SPXnYpJhsopTzEsBBwFHuiRQbXerPQtH';

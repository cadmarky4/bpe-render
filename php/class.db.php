...
abstract class db {
	 ...
	 * @return mixed
	 */ 
	public static function open(mixed $type, mixed $database, string $user = '', string $password = '', string $host = 'localhost', int $port = 33860, int $Log_Slow_DB_Query_Seconds = 0) { // Changed from 3306 to 33860
		$db_new = false;
		switch($type) {
			//case 'mysql':
			case 'mysqli':
...
class db_mysqli extends db {
	 ...
	 * @return bool|mysqli
	 */
	protected function _open(string $database, string $user, string $password, string $host, int $Log_Slow_DB_Query_Seconds, int $port = 33860) { // Changed from 3306 to 33860
